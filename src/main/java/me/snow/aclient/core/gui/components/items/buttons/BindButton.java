...
extends Button {
    ...
    public void drawScreen(int n, int n2, float f) {
        if (ClickGui.getInstance().rainbowRolling.getValue().booleanValue()) {
            ...
            RenderUtil.drawRect(this.x, this.y, this.x + (float)this.width + 7.4f, this.y + (float)this.height - 0.5f, this.getState() ? (!this.isHovering(n, n2) ? AliceMain.colorManager.getColorWithAlpha(AliceMain.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : AliceMain.colorManager.getColorWithAlpha(((ClickGui)ModuleManager.getModuleByName((String)"ClickGui")).alpha.getValue())) : (!this.isHovering(n, n2) ? 0x11555555 : -2007673515));
            if (ClickGui.getInstance().sideSettings.getValue().booleanValue()) {
                int n5 = ColorUtil.toARGB(ClickGui.getInstance().sidered.getValue(), ClickGui.getInstance().sidegreen.getValue(), ClickGui.getInstance().sideblue.getValue(), ClickGui.getInstance().sidealpha.getValue());
                RenderUtil.drawRect(this.x, this.y, this.x + 1.0f, this.y + (float)this.height + 1.0f, n5);
            }
        }
        if (this.isListening) {
            AliceMain.textManager.drawStringWithShadow("Press a key...", this.x + 2.3f, this.y - 4.0f - (float)LuigiGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
            AliceMain.textManager.drawStringWithShadow(String.valueOf(new StringBuilder().append(this.setting.getName()).append(" \u00a77").append(this.setting.getValue().toString())), this.x + 2.3f, this.y - 4.0f - (float)LuigiGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
