...
public final class GLUProjection {
    ...
    public boolean crossPlane(Vector3D[] arrvector3D, Vector3D vector3D) {
        Vector3D vector3D2 = new Vector3D(0.0, 0.0, 0.0);
        Vector3D vector3D3 = arrvector3D[1].sub(arrvector3D[0]);
        Vector3D vector3D4 = arrvector3D[2].sub(arrvector3D[0]);
        Vector3D vector3D5 = vector3D3.cross(vector3D4).snormalize();
        double d = vector3D2.sub(vector3D5).dot(arrvector3D[2]);
    ...
    public boolean[] doFrustumCheck(Vector3D[] arrvector3D, Vector3D vector3D, double d, double d2, double d3) {
        Vector3D vector3D2 = new Vector3D(d, d2, d3);
        boolean bl = this.crossPlane(new Vector3D[]{vector3D, arrvector3D[3], arrvector3D[0]}, vector3D2);
        boolean bl2 = this.crossPlane(new Vector3D[]{vector3D, arrvector3D[0], arrvector3D[1]}, vector3D2);
        boolean bl3 = this.crossPlane(new Vector3D[]{vector3D, arrvector3D[1], arrvector3D[2]}, vector3D2);
        boolean bl4 = this.crossPlane(new Vector3D[]{vector3D, arrvector3D[2], arrvector3D[3]}, vector3D2);
    ...
    public void updateMatrices(IntBuffer intBuffer, FloatBuffer floatBuffer, FloatBuffer floatBuffer2, double d, double d2) {
        ...
        double d4 = -vector3D3.y > 0.0 && d3 >= 90.0 && d3 < 270.0 || vector3D3.y > 0.0 && (!(d3 >= 90.0) || !(d3 < 270.0)) ? Math.toDegrees(Math.atan2(vector3D4.cross(vector3D2).length(), vector3D4.dot(vector3D2))) : -Math.toDegrees(Math.atan2(vector3D4.cross(vector3D2).length(), vector3D4.dot(vector3D2)));
    ...
    private GLUProjection() {
        this.coords = BufferUtils.createFloatBuffer((int)3);
    }
    ...
    public Projection project(double d, double d2, double d3, ClampMode clampMode, boolean bl) {
        ...
        boolean[] arrbl = this.doFrustumCheck(this.frustum, this.frustumPos, d, d2, d3);
        boolean bl3 = bl2 = arrbl[0] || arrbl[1] || arrbl[2] || arrbl[3];
        if (bl2) {
            boolean bl4;
            boolean bl5 = vector3D.sub(this.frustumPos).dot(this.viewVec) <= 0.0;
            boolean[] arrbl2 = this.doFrustumCheck(this.invFrustum, this.frustumPos, d, d2, d3);
            boolean bl6 = bl4 = arrbl2[0] || arrbl2[1] || arrbl2[2] || arrbl2[3];
            if (bl && !bl4 || bl4 && clampMode != ClampMode.NONE) {
                if (bl && !bl4 || clampMode == ClampMode.DIRECT) {
                    double d4;
                    double d5;
                    ...
                    if (bl5) {
                        d5 = this.displayWidth * this.widthScale - (double)this.coords.get(0) * this.widthScale - this.displayWidth * this.widthScale / 2.0;
                        d4 = this.displayHeight * this.heightScale - (this.displayHeight - (double)this.coords.get(1)) * this.heightScale - this.displayHeight * this.heightScale / 2.0;
                    } else {
                        d5 = (double)this.coords.get(0) * this.widthScale - this.displayWidth * this.widthScale / 2.0;
                        d4 = (this.displayHeight - (double)this.coords.get(1)) * this.heightScale - this.displayHeight * this.heightScale / 2.0;
                    }
                    Vector3D vector3D2 = new Vector3D(d5, d4, 0.0).snormalize();
                    d5 = vector3D2.x;
                    d4 = vector3D2.y;
                    Line line = new Line(this.displayWidth * this.widthScale / 2.0, this.displayHeight * this.heightScale / 2.0, 0.0, d5, d4, 0.0);
                    double d6 = Math.toDegrees(Math.acos(vector3D2.y / Math.sqrt(vector3D2.x * vector3D2.x + vector3D2.y * vector3D2.y)));
                    if (d5 < 0.0) {
                        d6 = 360.0 - d6;
                    }
                    new Vector3D(0.0, 0.0, 0.0);
                    Vector3D vector3D3 = d6 >= this.bra && d6 < this.tra ? this.rb.intersect(line) : (d6 >= this.tra && d6 < this.tla ? this.tb.intersect(line) : (d6 >= this.tla && d6 < this.bla ? this.lb.intersect(line) : this.bb.intersect(line)));
                ...
                if (d8 < 0.0) {
                    d8 = 0.0;
                    return new Projection(d7, d8, Projection.Type.OUTSIDE);
                }
    ...
    public static class Projection {
        ...
        public Projection(double d, double d2, Type type2) {
            this.x = d;
            this.y = d2;
            this.t = type2;
        }
