...
import me.snow.aclient.util.CombatUtil;
...
public class HoleUtilStay {
    ...
    public static boolean isInHole() {
        Vec3d vec3d = CombatUtil.interpolateEntity((Entity)HoleUtilStay.mc.player);
        BlockPos blockPos = new BlockPos(vec3d.xCoord, vec3d.yCoord, vec3d.zCoord);
        int n = 0;
        for (BlockPos blockPos2 : holeBlocks) {
            if (!CombatUtil.isHard(HoleUtilStay.mc.world.getBlockState(blockPos.add((Vec3i)blockPos2)).getBlock())) continue;
            ++n;
        }
        return n == 5;
    }
    ...
    public static boolean isHole(BlockPos blockPos) {
        ...
        for (BlockPos blockPos3 : holeBlocks) {
            if (!CombatUtil.isHard(HoleUtilStay.mc.world.getBlockState(blockPos2.add((Vec3i)blockPos3)).getBlock())) continue;
            ++n;
        }
        return n == 5;
    }
